...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--count", type=int, default=15, help="Количество продуктов для создания"
        )
        parser.add_argument(
            "--clear",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for i in range(count):
            ...
            product_name = (
                f"{brand} {template['name']} {model} {random.randint(100, 999)}"
            )
            ...
            category_desc = descriptions.get(
                category.name, ["Качественный товар по доступной цене."]
            )
            ...
            self.stdout.write(f"{status_icon} Создан продукт: {product.name[:40]}...")
            self.stdout.write(
        ...
        self.stdout.write(
            f"✅ В наличии: {available_products} ({available_products / total_products * 100:.1f}%)"
        )
        ...
        for category in Category.objects.all().order_by("name"):
            ...
            category_price_avg = (
                Product.objects.filter(category=category).aggregate(
                    models.Avg("price")
                )["price__avg"]
                or 0
            )
